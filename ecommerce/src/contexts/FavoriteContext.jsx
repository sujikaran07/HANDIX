...
export const FavoriteProvider = ({ children }) => {
  ...
  // Load favorites from API or localStorage on mount
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      try {
        if (isAuthenticated()) {
          // Fetch favorites from API for authenticated user
          ...
          // Fetch favorites from localStorage for guest user
  ...
  // Persist favorites to localStorage when changed (for guests and as backup)
  useEffect(() => {
    if (!loading) {
  ...
  // Add a product to favorites (API or localStorage)
  ...
  // Remove a product from favorites (API or localStorage)
...
export const useFavorites = () => {
  ...
  if (context === undefined) {
    ...
    return {
      ...
      clearFavorites: () => console.warn('FavoriteContext not available'),
      loading: false,
      error: null
    };
  }
